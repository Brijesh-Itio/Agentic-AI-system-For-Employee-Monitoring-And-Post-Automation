...
-- Notepad sidebar page (api/routes/notes.py). id is generated by the browser
-- so a note can be created offline and synced later; created_at/updated_at
-- are client epoch milliseconds, which the last-write-wins sync compares.
...
def update_social_post_fields(post_id: int, fields: dict) -> bool:
    """Updates only the listed columns (content/platform/source_url/
    image_url). A value of None writes NULL, which is how a field is cleared."""
    allowed = {"content", "platform", "source_url", "image_url"}
    cols = [c for c in fields if c in allowed]
    if not cols:
        return False
    with write_cursor() as cur:
        cur.execute(
            f"UPDATE seo_social_posts SET {', '.join(c + ' = ?' for c in cols)} WHERE id = ?",
            [fields[c] for c in cols] + [post_id],
        )
        return cur.rowcount > 0
