# WorkPulse AI — Module Specifications (historical)

> **Archive.** The original module-by-module build specification and status log, kept verbatim for reference.
> It records *why* each module was built the way it was, and the test results from the time.
> For the current architecture, setup and conventions read [`DEVELOPMENT.md`](../DEVELOPMENT.md); 
> for day-to-day use read [`USER_GUIDE.md`](USER_GUIDE.md). Modules 33 onward (the later SEO work) are
> documented in code comments tagged `Module N` and summarised in `DEVELOPMENT.md`, not here.
